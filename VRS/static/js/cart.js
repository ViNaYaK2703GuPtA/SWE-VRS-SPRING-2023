...
document.getElementById('search-button').addEventListener('click', function(event) {
    ...
    fetch('/get-products/')
        ...
        .then(data => {
            ...
            if(term.length ===  1){
                 matchingProducts = products.filter(product => product.name.toLowerCase().startsWith(term));
            }
            if(term.length > 1) {
                 matchingProducts = products.filter(product => product.name.toLowerCase().includes(term));
            }
